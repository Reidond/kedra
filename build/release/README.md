...
All OS/ISO production runs in Actions. [Published r1](https://github.com/Reidond/kedra/releases/tag/desktop-44-x86_64-r1) remains available; new source does not replace it until qualification and promotion complete. See [install](../../docs/INSTALL.md), [verification](../../docs/RELEASES.md) and [observed status](../../docs/STATUS.md).
...
Missing legacy comparison material conservatively requires a candidate. R1 lacks this resolved-input schema, so the first midnight refresh builds a candidate; its later qualified promotion establishes the baseline for future no-change checks.
