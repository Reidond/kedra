...
a, b = signed(release('A', 1)), signed(release('B', 5))
envelope('status', 'status')
envelope('enroll', 'enroll', a, 1)
envelope('stage-b', 'stage', b, 5, replace_staged=None, resume=False)
envelope('rollback-a', 'rollback', a, replace_staged=None)
envelope('resume-b', 'stage', b, 5, replace_staged=None, resume=True)
envelope('replay-a', 'stage', a, 1, replace_staged=None, resume=False)
envelope('expired', 'stage', b, 5, expired=True, replace_staged=None, resume=False)
for variant, sequence in [('U', 2), ('W', 3), ('M', 4)]:
    envelope(f'oci-{variant}', 'stage', signed(release(variant, sequence)), sequence, replace_staged=None, resume=False)
envelope('pending', 'stage', signed(release('U', 6)), 6, replace_staged=None, resume=False)
envelope('wrong-key', 'stage', signed(release('B', 5), 'wrong'), 5, replace_staged=None, resume=False)
envelope('wrong-target', 'stage', signed(release('B', 5, scope={**scope, 'target': 'other'})), 5, replace_staged=None, resume=False)
envelope('candidate', 'stage', signed(release('B', 5, approval='candidate')), 5, replace_staged=None, resume=False)
