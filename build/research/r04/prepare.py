...
if args.phase == 'images':
    ...
    for variant, directory, text in [('A', repo, original), ('B', source_b, incoming)]:
        ...
        for name in ['Containerfile', 'check.sh', 'check.service', 'home.py']:
            shutil.copyfile(repo / 'build/research/r04' / name, context / name)
        shutil.copyfile(repo / 'build/research/console.toml', context / 'console.toml')
        shutil.copyfile(repo / 'build/research/r07/test-profile.toml', context / 'test-profile.toml')
