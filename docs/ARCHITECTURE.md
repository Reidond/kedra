...
Build jobs have public trust only. The protected signer executes no checkout, candidate binaries or repository scripts with production keys. Per-target serialization and exact-source checks prevent stale stable-tag publication. Changed image inputs require manual OCI-signing review. No-change does nothing; there is no checkpoint renewal. Uncertain registry publication requires exact readback before reporting success.
