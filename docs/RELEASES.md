...
Old release-file verification is retained only for locally saved legacy/offline records. The r1, r2 and legacy-channel GitHub Releases and all 31 uploaded assets have been removed; their source Git tags remain. Existing legacy clients cannot download the deleted channel. Complete the [legacy migration](UPDATES.md#one-time-legacy-migration) before relying on the GHCR updater. [STATUS](STATUS.md) records actual publication and migration evidence; no current stable publication is implied by this policy change.
