...
Current source includes channel discovery and home-status features absent from r1. Midnight resolved-input refresh, protected renewal, fixed-channel enrollment/staging and repository consolidation are locally committed at `3c6c876` and awaiting push/Actions. No push, new Actions run or publication occurred with that commit; its working tree was clean immediately afterward. These changes are not retroactively part of r1.
