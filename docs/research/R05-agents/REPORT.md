...
Date: 2026-09-08. Launcher Linux tests: not-run, pending Actions. Full gate blocked.
...
Prepared seven Linux selection/profile/environment tests plus an actual CLI
process test for lock inheritance, exit status and synthetic vault exclusion.
Windows fmt/Clippy and existing workspace tests pass; Linux-specific behavior is
not qualified by Windows checks. Local Cargo initially selected an inherited
...
Remaining: actual Linux results, official binary write/discovery traces, full
