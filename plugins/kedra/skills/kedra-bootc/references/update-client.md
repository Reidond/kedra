...
The fixed installed helper validates native OCI signature/repository/target, image-owned identity and retained ordering. User checkouts are irrelevant to authority. Each verification copies from the registry again, into a root-only OCI layout that reuses cached layers. Only the first verification downloads the whole image. See docs/UPDATES.md. Rollback holds forward updates; explicit stage --resume clears the hold after normal checks. Persistent home and /var are not rewound.
