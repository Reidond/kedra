...
Read docs/ARCHITECTURE.md. Agents and home operations run as the ordinary user. The CLI requests administrator authorization for a fixed image-owned helper; no setuid bit or passwordless writable-script wrapper. The helper independently verifies fixed trust, scope, eligibility, freshness and ordering.
