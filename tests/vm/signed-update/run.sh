...
EOF
python3 tests/vm/signed-update/helper-fixtures.py prepare --root "$root"
...
cp tests/vm/signed-update/{Containerfile,check.sh,check.service,install.toml} "$root/context/"
...
python3 tests/vm/signed-update/helper-fixtures.py requests --root "$root"
