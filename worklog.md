...
|---|---|
| Phase | Implementation underway: signed update/rollback, graphical desktop, signed research installation and real A/B/A home transitions pass. Dedicated owner release authority is provisioned; no promoted owner installer yet. |
...
| Latest verified check | Main 34255226140 and development 34285217784 at 0626e6d pass. R01 34253906774 passes signed native registry/storage and update/rollback. Replacement 34255228394 at accepted main c660c58 passes public build, GHCR/storage guard, owner-approved signing of bb4f2b68, anonymous native verification, complete ISO construction and offline payload/Anaconda startup. Complete media is downloading for fresh local installation. Prior signed research installation and real A/B/A retain their exact scopes. |
...
- Checks / evidence: pass — exact candidate/source and pending environment readback; approved deployment API response. in-progress — isolated signing and subsequent installer workflow. not-run — replacement ISO, fresh owner-media VM installation and promotion.
- Remaining / blockers: No owner ISO/channel is promoted. Actual signed anonymous pull, v82 ISO construction and complete encrypted installation still need evidence; wider lifecycle and hardware gates remain open.
- Next: Follow signing and ISO construction, verify complete resulting media and perform the prepared two-disk encrypted installation with exact installed health/provenance checks.
...
- Agent / state: Codex; in-progress, implementation prepared for Actions.
...
- Checks / evidence: pass — Python AST, JSON, YAML and workflow Bash syntax; independent read-only review. The review found and corrected refusal classification that could accept unrelated DNF errors; negative cases now require specific native evidence, including exact retained corrupted archive bytes and RPM verification. not-run — every native case until the new Actions workflow executes. See docs/research/update-refresh/REPORT.md and results.json.
- Remaining / blockers: Native Fedora compatibility, diagnostic wording and outcomes are unqualified. Full OS equivalence, schedule/renewal, promotion, ISO/boot and other supplemental cases remain open. Disposable signing keys stay outside build contexts/images/upload paths; no production key or workstation package action is used.
- Next: Push the prepared development-only paths, inspect actual native evidence and correct observed failures without weakening the solver/signature/equivalence boundaries.
